...
| `custom:star-projector-popup-card` | Sternenprojektor (Popup) | Same controls, but the chevron opens them in a **modal popup** (`<dialog>`). Every entity is an individually selectable field in the editor (no prefix). |
...
The tune icon in the header opens the controls in a centered modal popup (Esc / backdrop /
✕ to close). The editor presents each entity as its own picker.
